...
class Base16Codec implements Codec {
	private static final int OFFSET_OF_LOWER_A = 'a' - 10;
	private static final int OFFSET_OF_UPPER_A = 'A' - 10;
		...
		private static byte[] decodeTable() {
			...
			for (int i = 0; i <= 'f'; i++) {
				if (i >= '0' && i <= '9')
					...
					dest[i] = (byte) (i - OFFSET_OF_UPPER_A);
				else if (i >= 'a' && i <= 'f')
					dest[i] = (byte) (i - OFFSET_OF_LOWER_A);
				else
					dest[i] = -1;
			}
			return dest;
	...
	public byte[] encode(byte[] src) {
		...
		for (int i = 0; i < src.length; i++, j += 2) {
			...
			dest[j + 1] = alphabets[p & MASK_4BITS];
		}
		return dest;
	}
	...
	public byte[] decode(byte[] src, final int length) {
		...
		for (int i = 0, j = 0; i < length; i += 2, j++) {
			dest[j] = (byte) ((pos(src[i]) << 4) | pos(src[i + 1]));
		}
		return dest;
	}
