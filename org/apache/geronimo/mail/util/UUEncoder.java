...
public class UUEncoder implements Encoder {
	...
	// this is the maximum number of chars allowed per line, since we have to
	// include a uuencoded length at
	...
	private int encodeLine(byte[] data, int offset, int length, OutputStream out) throws IOException {
		...
		for (int i = 0; i < length; i += 3) {
			...
			byte d3 = (byte) ((((b << 2) & 0x3C) | ((c >>> 6) & 0x03)) + ' ');
			byte d4 = (byte) ((c & 0x3F) + ' ');
	...
	public int decode(byte[] data, int off, int length, OutputStream out) throws IOException {
		...
		while (length > 0) {
			...
			// the offset was left pointing at the EOL character, so step over that one
			// before
			// scanning again.
			off++;
			length--;
	...
	/**
	 * decode a single line of uuencoded byte data writing it to the given output
	 * stream
	 *
	 * @param data   The array of byte data to decode.
	 * @param off    Starting offset within the array.
	 * @param length The length of data to decode (length does NOT include the
	...
	private int decodeLine(byte[] data, int off, int length, OutputStream out) throws IOException {
		...
		// get the rounded count of characters that should have been used to encode
		// this. The + 1 is for the
		...
		while (bytesWritten < count) {
			...
			// still have more bytes to decode? do the second byte of the second. That
			// requires
